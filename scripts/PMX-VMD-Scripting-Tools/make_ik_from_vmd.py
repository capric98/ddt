#!/usr/bin/env python3
# coding: utf-8
# Copy from github.com/Nuthouse01/PMX-VMD-Scripting-Tools
...
import core.nuthouse01_core as core
import core.nuthouse01_pmx_parser as pmxlib
import core.nuthouse01_pmx_struct as pmxstruct
import core.nuthouse01_vmd_parser as vmdlib
import core.nuthouse01_vmd_struct as vmdstruct
from core.nuthouse01_pmx_utils import bone_get_ancestors
from core.nuthouse01_vmd_utils import remove_redundant_frames, fill_missing_boneframes, dictify_framelist
...
def main(moreinfo=True):
    ...
    parser = argparse.ArgumentParser(prog="make_ik_from_vmd.py")

    parser.add_argument("-s", "--source", type=str, help="source PMX model", required=True)
    parser.add_argument("-t", "--target", type=str, help="target PMX model", required=True)
    parser.add_argument("-m", "--motion", type=str, help="animation VMD file", required=True)
    parser.add_argument("bone_pair", type=str, nargs="?", default=["左足ＩＫ/左足首", "右足ＩＫ/右足首", "左つま先ＩＫ/左足先EX", "右つま先ＩＫ/右足先EX"])

    args = parser.parse_args()
    args = {**vars(args)}

    bone_pair = args["bone_pair"]
    print(bone_pair)
    ...
    input_filename_pmx_source = args["source"]
    ...
    input_filename_pmx_dest = args["target"]
    ...
    try:
        header_name = pmx_dest.header.name_jp
    except:
        try:
            header_name = pmx_dest.header.name_en
        except:
            header_name = os.path.splitext(os.path.basename(args["target"]))[0]

    vmd_out = vmdstruct.Vmd(
        header=vmdstruct.VmdHeader(version=2, modelname=header_name),
